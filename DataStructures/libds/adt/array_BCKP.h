...
namespace ds::adt {
    ...
    ADT& Array<T>::assign(const ADT& other)
    {
        ...
        if (other == nullptr)
        {
            throw std::runtime_error("Nie je array");
        }

        if (base_ != otherArray->base_)
        {
            throw std::runtime_error("Nie je array");
        }
    ...
    T Array<T>::access(long long index) const
    {
        ...
        {
            throw std::out_of_range("Zle");
        }
        return this->getSequence()->access(this->mapIndex())->data_;
    }
    ...
    {
        return index >= base_ && index < base_ ? this->size();
    }
