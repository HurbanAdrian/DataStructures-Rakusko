...
namespace ds::adt {
    ...
    public:
        ...
        void insert(const K& key, T data) override;
        void insert(const K& key, T data, bool dup);
        bool tryFind(const K& key, T*& data) const override;
        T remove(const K& key) override;
    ...
    template<typename K, typename T, typename ItemType>
    inline void GeneralBinarySearchTree<K, T, ItemType>::insert(const K& key, T data, bool dup)
    {
        ...
        if (this->isEmpty()) {
            ...
            BSTNodeType* parent = nullptr;
            if (this->tryFindNodeWithKey(key, parent)) {                //existuje kluc pridavame duplicitu
                parent->data_.data_.pridajDuplikat(data.getPovodna());
                return;
            }
            // podla poradia
    ...
    void GeneralBinarySearchTree<K, T, ItemType>::removeNode(BSTNodeType* node)
    {
        ...
        BSTNodeType* prechodcaVPoradi = this->getHierarchy()->accessLeftSon(*node);
        BSTNodeType* syn = this->getHierarchy()->hasLeftSon(*node) ? this->getHierarchy()->accessLeftSon(*node) : this->getHierarchy()->accessRightSon(*node);
        ...
        case 1:
            ...
            if (this->getHierarchy()->accessLeftSon(*node) == syn)
            {
        ...
        case 2:
            ...
            std::swap(node->data_, prechodcaVPoradi->data_);                // este skontrolovat        !!!!!!
    ...
    {
        BSTNodeType* lavySyn = this->getHierarchy()->accessLeftSon(*node);
        BSTNodeType* otec = this->getHierarchy()->accessParent(*node);
        BSTNodeType* praOtec = this->getHierarchy()->accessParent(*otec);
        ...
        if (praOtec != nullptr)
        {
            if (this->getHierarchy()->accessLeftSon(*praOtec) == otec)
            {
    ...
    {
        BSTNodeType* pravySyn = this->getHierarchy()->accessRightSon(*node);
        BSTNodeType* otec = this->getHierarchy()->accessParent(*node);
        BSTNodeType* praOtec = this->getHierarchy()->accessParent(*otec);
    ...
    void Treap<K, T>::removeNode(BSTNodeType* node)
    {
        ...
        GeneralBinarySearchTree<K, T, TreapItem<K, T>>::removeNode(node);                           // kontrola
    }
    ...
    void Treap<K, T>::balanceTree(BSTNodeType* node)
    {
        ...
        while (otec != nullptr && otec->data_.priority_ > node->data_.priority_) {
            ...
            otec = this->getHierarchy()->accessParent(*otec);
        }
    }
    ...
    template <typename K, typename T>
    class MultiValueTable : public SortedSequenceTable<K, T> {                    // zmenili by sme na treap potom
    private:
        size_t size_;
    public:

        void insert(const K& key, const T& data) {
            //using Seq = typename amt::ImplicitSequence<TableItem<K, T>>;
            using BlockType = typename amt::IS<TableItem<K, T>>::BlockType;
            ...
            if (this->isEmpty()) {
                ...
                BlockType* blok = nullptr;
                bool found = this->tryFindBlockWithKey(key, 0, this->size(), blok);
                if (found) {
                    //blok->data_.data_.pridajDuplikat(data);
                    blok->data_.data_.pridajDuplikat(data.getPovodna());
                    return;
                }
                // podla poradia
                if (key > blok->data_.key_) {
                    tableItem = &this->getSequence()->insertAfter(*blok).data_;
                }
                else {
                    tableItem = &this->getSequence()->insertBefore(*blok).data_;
                }
            }
    ...
    /*
    template <typename K, typename T>
    class MultiValueTable : public Treap<K, T> {                    // zmenili sme na treap potom
    private:
        size_t size_;
    public:
        void insert(const K& key, const T& data) {
            //using Seq = typename amt::ImplicitSequence<TableItem<K, T>>;
            using BSTNodeType = typename Treap<K, T>::BSTNodeType;
