...
namespace ds::adt {
    ...
    {
        auto* najlepsiBlok = this->getSequence()->accessFirst(); 
        this->getSequence()->processAllBlocksForward(getSequence()->accessNext(*najlepsiBlok), [&](SequenceType::BlockType* b) {
            if (najlepsiBlok->data_.priority > b->data_.priority)
            {
                najlepsiBlok = b;
            }
            });
    ...
    {
        PQItem<P, T>* dataFrontu = nullptr;
        if (isEmpty() && priority <= this->getSequence()->accessLast()->data_->priority)
        {
        ...
        {   // asi takto?
            dataFrontu = (this.getSequence()->findBlockWithProperty(
                [&](auto* b) { return b->data_.priority >= priority; }
            );
            dataFrontu = &this->getSequence()->insertBefore(*block)->data_;
        }
    ...
    {
        PQItem<P, T>& pqi = this->getHierarchy()->insertLastLeaf().data_;
        ...
        while (parentBlock != nullptr && currentBlock->data_.priority < parentBlock->data_.priority) {
            std::swap(currentBlock->data_, parentBlock->data_);
            currentBlock = parentBlock;
            parentBlock = getHierarchy()->accessParent(*currentBlock);
        }
    }
