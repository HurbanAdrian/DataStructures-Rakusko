...
namespace ds::adt
{
    ...
    public:
        ...
        void sort(amt::ImplicitSequence<T>& is, std::function<bool(const T&, const T&, int, std::string)> compare, int rok, std::string pohlavie);

        // s pomcou stackOverflow a yt
        void knuthSort(amt::ImplicitSequence<T>& is, std::function<bool(const T&, const T&)> compare)
        {
            size_t n = is.size();
            size_t gap = 1;
            while (3 * gap + 1 < n) {           // dopocitame gap
                gap = 3 * gap + 1;
            }

            while (gap > 0) {                               // do 1 by malo ist
                for (size_t i = gap; i < n; ++i) {                      // porovnavame ho ak mensi posunieme vlavo
                    size_t j = i;
                    while (j >= gap && compare(is.access(i)->data_, is.access(j - gap)->data_)) {
                        std::swap(is.access(j)->data_, is.access(j - gap)->data_);
                        j = j - gap;
                    }
                }
                gap = gap / 3;
        ...
        void knuthSort(amt::ImplicitSequence<T>& is, std::function<bool(const T&, const T&, int, std::string)> compare, int rok, std::string pohlavie)
        {
            size_t n = is.size();
            size_t gap = 1;
            while (3 * gap + 1 < n) {
            ...
            while (gap > 0) {
                for (size_t i = gap; i < n; ++i) {
                    size_t j = i;
                    while (j >= gap && compare(is.access(j)->data_, is.access(j - gap)->data_, rok, pohlavie)) {
                        std::swap(is.access(j)->data_, is.access(j - gap)->data_);
                        j = j - gap;
                    }
                }
    ...
    private:
        ...
        void shell(amt::ImplicitSequence<T>& is, std::function<bool(const T&, const T&, int, std::string)> compare, size_t k, int rok, std::string pohlavie);
    };
    ...
    void ShellSort<T>::shell(amt::ImplicitSequence<T>& is, std::function<bool(const T&, const T&)> compare, size_t k)
    {
        ...
        if (k > 1)
        {
            shell(is, compare, (k - 1) / 3);
            //shell(is, compare, k - 1);
        }
    }

    template<typename T>
    void ShellSort<T>::sort(amt::ImplicitSequence<T>& is, std::function<bool(const T&, const T&, int, std::string)> compare, int rok, std::string pohlavie)
    {
        size_t n = is.size();
        size_t k = 1;
        while (3 * k + 1 < n) {
        ...
        shell(is, compare, k, rok, pohlavie);
    }

    template<typename T>
    void ShellSort<T>::shell(amt::ImplicitSequence<T>& is, std::function<bool(const T&, const T&, int, std::string)> compare, size_t k, int rok, std::string pohlavie)
    {
        for (size_t d = 0; d < k; d++)
        {
            for (size_t i = d; i < is.size(); i++)
            {
                size_t j = i;
                while (j >= k && (j - k) >= d && compare(is.access(j)->data_, is.access(j - k)->data_, rok, pohlavie))
                {
                    std::swap(is.access(j)->data_, is.access(j - k)->data_);
                    j = j - k;
                }
        ...
        if (k > 1)
        {
            shell(is, compare, (k - 1) / 3, rok, pohlavie);
        }
    }
