...
namespace ds::adt {
    ...
    {
        size_t vysledok = 0;
        auto* block = this->getSequence()->findBlockWithProperty([&] (BlockType* block) {
        ...
        return std::length_error("Neplatny index");
    }
    ...
    {
        auto* block = this->getSequence()->accessFirst();       // nevieme ci bude jednosmerne obojsmerne zretazeny blok alebo ako tak preto auto

        if (block == nullptr)
        {
            throw std::range_error("Struktura je prazdna");
        }
        else
        {
            return block->data_;
    ...
    void GeneralList<T, SequenceType>::set(size_t index, T element)
    {
        ...
        {
            throw std::invalid_argument("Nespravny index");
        }
