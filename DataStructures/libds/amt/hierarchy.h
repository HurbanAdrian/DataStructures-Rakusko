...
namespace ds::amt {
	...
	{	// ideme z lava do prava, napravo najvacsie (rovno aj zoradene),
		if (node != nullptr)
		{	// mohli by sme prerobit aj rovno keby pre a post podla posunutia toho operation
	...
	{
		if (!this->currentPosition_->currentNodeProcessed_)
		{	// najskor laveho pozzrieme a ci sa da ist este v lavo
		...
		else          // Navysujeme indexy pri nich
		{	// pravy a ci sa da ist este viacej vpravo
			...
			{	// inak opdstranime a ak nie sme na NULL tak zvysime index
				this->removePosition();
				if (this->currentPosition_ != nullptr)
				{
					++(*this);
