...
namespace ds::amt {
	...
	public:
		...
		// pre order 1->2 post je 2->1
		// post order mazanie pre zase kopirovanie...
	...
	{
		int uroven = 0;
		BlockType* otec = accessParent(node);
		while (otec != nullptr) {
			uroven++;
			otec = accessParent(*otec);			// musi byt * pred otec a nie za!
		}
		return uroven;
	}
	...
	{
		BlockType* otec = accessParent(node);
		return otec != nullptr && accessSon(*otec, sonOrder) == &node;
	}
    ...
    void Hierarchy<BlockType>::processPreOrder(const BlockType* node, std::function<void(const BlockType*)> operation) const
	{
		if (node != nullptr)
		{
			//1 krok vykonanie akcie
			operation(node);
			//2 krok vykonanie prehliadky na mnozine synov
			int stupenVrcholu = degree(*node);
			int poradieSyna = 0;
			int pocetSpracovanychSynov = 0;
			while (pocetSpracovanychSynov < stupenVrcholu) {
				BlockType* syn = accessSon(*node, poradieSyna);
				if (syn != nullptr)
				{
    ...
    void Hierarchy<BlockType>::processPostOrder(BlockType* node, std::function<void(BlockType*)> operation) const
	{
		if (node != nullptr)
		{
			...
			while (pocetSpracovanychSynov < stupenVrcholu) {
				...
				if (syn != nullptr)
				{
					processPostOrder(syn, operation);
					pocetSpracovanychSynov++;
				}
				poradieSyna++;
			}
