...
namespace ds::amt {
	...
	public:
		...
		// iba tu vieme naimplementovat? ze nevyhadzuje problemy
	...
	{
		int aktualnaUroven = level(index);
		int indexPosledneho = size() - 1;
		int hlbka = level(indexPosledneho);
		if (aktualnaUroven == hlbka)
		{
			return 0;
		}
		else
		{
			if (aktualnaUroven == hlbka - 1)
			{
				int indexOtcaPosledneho = indexOfParent(indexPosledneho);
				if (index < indexOtcaPosledneho)
				{
					return K;
				}
				else
	...
	{
		const size_t index = indexOfParent(node);
	...
	{
		const size_t index = indexOfSon(node, sonOrder);
