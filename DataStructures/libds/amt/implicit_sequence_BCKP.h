...
namespace ds::amt {
	...
	typename ImplicitSequence<DataType>::BlockType* ImplicitSequence<DataType>::access(size_t index) const
    {
		return index >= 0 && index < this->size() ?
			& this->getMemoryManager()->getBlockAt(index) :			// & na index
			nullptr;		
	}
    ...
    {
		int indexPredchodcu = indexOfPrevious(this->getMemoryManager()->calculateIndex(block));
		if (indexPredchodcu != INVALID_INDEX)
		{
			return &this->getMemoryManager()->getBlockAt(indexPredchodcu);
		}
		else
		{
			return nullptr;
    ...
    {
		return *this->getMemoryManager()->allocateMemoryAt(0);					// vracia to pointer my to chceme dereferencovat a preco davame * na referenciu
	}
	...
	{
		return this->getMemoryManager()->changeCapacity(capacity);						// cez -> treba			7segmentovy robime to iste ze obalujeme obdlznik vela metod => segment mal iba par
	}
    ...
    size_t ImplicitSequence<DataType>::indexOfNext(size_t currentIndex) const
	{
		...
		*/
		...
		return currentIndex >= this->size() + 1 ? INVALID_INDEX : currentIndex + 1;				// !!! this -> size !!! NIE !!! this . size

		// return currentindex >= this.size() + 1 ? INAVILD INDEX : current index + 1
	}
