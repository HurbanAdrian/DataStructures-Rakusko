...
namespace ds::amt {
    ...
    void ExplicitSequence<BlockType>::clear()
    {
        ...
        while (first_ != nullptr) {
            ...
            //ds::mm::MemoryManager<BlockType>().release(last_);
    ...
    {
        if (this == &other)         // namiesto toho otherEs 2 podmienky
        {
            return true;
        }
        const ExplicitSequence<BlockType>* otherES = dynamic_cast<const ExplicitSequence<BlockType>*>(&other); // dynamic cast je pretypovanie a ked bereme pointre tak bud vratu nullptr alebo 
        if (otherES == nullptr)                                                                             // v c++a to nedokazeme pretypovat static castom a tak preto mame dynamicCast
        {
            return false;
        }
        if (this == otherES)
        ...
        BlockType* mojAktualny = this->accessFirst();
        BlockType* inyAktualny = otherES->accessFirst();
        while (mojAktualny != nullptr) {
            if (!(mojAktualny->data_ == inyAktualny->data_))
            {
                return false;
            }
            else
            {
                mojAktualny = accessNext(*mojAktualny);
                inyAktualny = otherES->accessNext(*inyAktualny);
            }
            return true;
        }
    }
    ...
    {
        int vysledok = 0;
        BlockType* blok = this->findBlockWithProperty([&](BlockType* b)->bool {              //[] lambda         [] co chcem poznat - teraz vsetko () typovo kompatibilne s tym co budu posielat {}
                vysledok++;                                                           // {}
                return &data == b;                                                      // ->bool tam teoreticky netreba ono to skontroluje ten predikat
            }                             
        );

        return blok != nullptr ? vysledok - 1 : INVALID_INDEX;
        /*if (blok != nullptr)
        {
            return vysledok - 1;
        }
    ...
    BlockType* ExplicitSequence<BlockType>::access(size_t index) const
    {
        ...
        BlockType* vysledok = nullptr;
        if (index >= 0 && index <= this->size())
        {
            vysledok = accessFirst();
            int i = 0;
            while (i < index) {
                vysledok = this->accessNext(*vysledok);

                // mozeme aj takto vysledok = static_cast<BlockType*>(vysledok->next_); nakolko nechcem viac virtualnych volani robit
    ...
    {
        if (this->size() == 0)                                              // !!! this->siez() NIE !!! this.size()
        {
            this->first_ = this->last_;
            this->last_ = memoryManager_->allocateMemory();
            return *first_;
        }
    ...
    {
        BlockType* nasledujuciBlok = accessNext(block);
        BlockType* novyBlok = memoryManager_->allocateMemory();
        connectBlocks(&block, novyBlok);
        connectBlocks(novyBlok, nasledujuciBlok);
        if (last_ == &block)
        {
            last_ = novyBlok;
        }
    ...
    {
        BlockType* predchadzajuciBlok = accessPrevious(block);
        BlockType* novyBlok = memoryManager_->allocateMemory();
        connectBlocks(predchadzajuciBlok, novyBlok);
        connectBlocks(novyBlok, &block);
        if (first_ == &block)
        {
            first_ = novyBlok;
        }
