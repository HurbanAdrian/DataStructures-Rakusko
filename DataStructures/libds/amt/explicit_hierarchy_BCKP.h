...
namespace ds::amt {
	...
	{
		processPostOrder(root_, [&](BlockType* b) {
	...
	{
		size_t count = 0;
		processPreOrder(root_, [&](const BlockType* b) {		//musi byt const blockType
			count++;
			});
		return count;
	}
	...
	{
		return static_cast<BlockType*>(node.parent_);		// nevidime parenta -> neviem typ ale my mu povieme
	}														// ze je to ok. Jemu vadi ze parent moze byt cokolvek
	...
	{
		MemoryBlock<BlockType*> blokSynov = node.sons_->access(sonOrder);
