...
namespace ds::mm {
    ...
    {
        return this->allocateMemoryAt(static_cast<size_t>(end_ - base_));           // potencionalne moze povedat ze porovnavame zle veci keby nebolo static cast
    }
    ...
    BlockType* CompactMemoryManager<BlockType>::allocateMemoryAt(size_t index)
    {
        ...
        return placement_new(base_ + index);        // return new (base_ + index) BlockType();          to ISTE
    }
