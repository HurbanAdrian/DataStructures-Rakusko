...
namespace ds::mm {
		...
		ptr_(ptr)
	{
		...
		for (size_t byte = 0; byte < sizeof(DataType); byte++, ptrB++) {		//treba dereferencovat ptrB na *ptrb
			if (*ptrB >= 'a' && *ptrB <= 'z' || *ptrB >= 'A' && *ptrB <= 'Z')			// (*ptrB >= 'a' && *ptrB <= 'z' || *ptrB >= 'A' && *ptrB <= 'Z')			(*ptrB >= 41 && *ptrB <= 122)
			{
				oss_ << "|" << (char)*ptrB;
			}
			else
